...
public:
  ...
  virtual bool enqueue(const unipath_manager* source, detail::item_scope scope,
...
template <>
struct hash<broker::internal::unipath_manager_ptr> {
  using argument_type = broker::internal::unipath_manager_ptr;
  size_t operator()(const argument_type& x) const noexcept {
    hash<broker::internal::unipath_manager*> f;
    return f(x.get());
  }
};
