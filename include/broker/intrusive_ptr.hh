...
intrusive_ptr<T> make_intrusive(Ts&&... args) {
  ...
  return {adopt_ref, new T(std::forward<Ts>(args)...)};
}

/// Casts an @c intrusive_ptr object to another by way of static_cast on the
/// underlying pointer.
/// @param p  The pointer of type @c U to cast to another type, @c T.
/// @return  The pointer, as cast to type @c T.
template <class T, class U>
intrusive_ptr<T> cast_intrusive(intrusive_ptr<U> p) noexcept {
  return {adopt_ref, static_cast<T*>(p.release())};
}
