...
struct builder_access {
  ...
  static Builder& add(Builder& builder, const T& value) {
    if constexpr (is_builder<T>) {
      ...
      format::bin::v1::write_sequence(value.tag(), value.num_values(), first,
                         ...
                         || std::is_same_v<data, T>) {
      format::bin::v1::encode(value, builder.adder());
    } else {
      format::bin::v1::write_unsigned(data_tag_v<T>, builder.adder());
      format::bin::v1::encode(value, builder.adder());
    }
    return builder;
  }
