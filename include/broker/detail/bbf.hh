...
/// BBF stands for "broker binary format". This namespace contains all
/// functionality related to serializing and deserializing Broker's data types
...
template <class OutIter>
using value_t = typename value_type_oracle<OutIter>::type;
...
OutIter write_varbyte(size_t value, OutIter out) {
  return write_varbyte_impl(value, [out](auto* begin, auto* end) {
    using value_type = value_t<OutIter>;
...
OutIter write_unsigned(T value, OutIter out) {
  if constexpr (std::is_enum_v<T>) {
    ...
    value_t<OutIter> buf[sizeof(T)];
    memcpy(buf, &tmp, sizeof(T));
    return std::copy(buf, buf + sizeof(T), out);
  } else {
    *out++ = static_cast<value_t<OutIter>>(value);
    return out;
  }
}
...
template <class OutIter>
OutIter encode(const data_view_value::vector_view* values, OutIter out) {
  out = write_unsigned(data::type::vector, out);
  out = write_varbyte(values->size(), out);
  for (const auto& x : *values)
    out = encode(x, out);
  return out;
