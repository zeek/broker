...
int main(int argc, char** argv) {
  ...
  BROKER_TEST(
    check_contents_poll(pq_a0, {broker::message{"0a", "node0 says: hi"},
  ...
  BROKER_TEST(
    check_contents(pq_b0, {broker::message{"0b", "node0 says: bye"},
                           broker::message{"0b", "node0 says: goodbye"},
                           broker::message{"1b", "node1 says: bbye"},
                           broker::message{"2b", "node2 says: bbye"}}));

  BROKER_TEST(
    check_contents_poll(pq_a1, {broker::message{"0a", "node0 says: hi"},
                                broker::message{"0a", "node0 says: hello"},
                                broker::message{"1a", "node1 says: hi"},
                                broker::message{"1a", "node1 says: bye"}}));

  BROKER_TEST(
    check_contents(pq_b1, {broker::message{"0b", "node0 says: bye"},
                           broker::message{"0b", "node0 says: goodbye"},
                           broker::message{"1b", "node1 says: bbye"}}));

  BROKER_TEST(
    check_contents_poll(pq_a2, {broker::message{"0a", "node0 says: hi"},
                                broker::message{"0a", "node0 says: hello"},
                                broker::message{"2a", "node2 says: hi"},
                                broker::message{"2a", "node2 says: bye"}}));
