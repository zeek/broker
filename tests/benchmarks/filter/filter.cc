...
/// A trie that uses 256 children per node, i.e., uses the full ASCII range.
class trie256 {
public:
  // -- member types -----------------------------------------------------------
  ...
  void insert(std::string_view str) {
    ...
    for (auto c : str) {
      ...
      if (!ptr->children[index]) {
        ...
        ptr->children[index] = std::make_unique<leaf>(std::string{str});
        return;
      }
      ...
      if (next->is_leaf()) {
        ...
        std::ignore = ptr->children[index].release();
