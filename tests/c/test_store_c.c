...
int compare(broker_store_frontend* f, broker_table* expect) {
  ...
  broker_store_result* res
    = broker_store_frontend_request_blocking(f, key_query);
  BROKER_TEST(broker_store_result_get_status(res)
              == broker_store_result_status_success);
...
int main() {
  ...
  broker_store_frontend* m
    = broker_store_master_create_memory(node, master_name);
  ...
  for (i = 0; i < 3; ++i) {
    ...
    BROKER_TEST(
      broker_store_frontend_insert(m, entries[i].key, entries[i].val));
  }

  broker_store_frontend* c
    = broker_store_clone_create_memory(node, master_name, .2);
  ...
  BROKER_TEST(broker_store_result_get_status(size_res)
              == broker_store_result_status_success);
  BROKER_TEST(broker_store_result_count(size_res)
              == broker_table_size(dataset));
