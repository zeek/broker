...
broker_data* make_test_msg(broker_endpoint* node, const char* path,
  ...
  BROKER_TEST(broker_string_set_cstring(
    broker_data_as_string(broker_vector_lookup(msg, 0)), path));
  BROKER_TEST(broker_string_set_cstring(
    broker_data_as_string(broker_vector_lookup(msg, 1)), str));
...
int main() {
  ...
  int off = 0;
  msg_data = make_test_msg(node0, "0a", "node0 says: hi", topic_a, msg,
                           msgs_to_send + off++);
  ...
  msg_data = make_test_msg(node0, "0a", "node0 says: hello", topic_a, msg,
                           msgs_to_send + off++);
  BROKER_TEST(broker_set_insert(pq_a0_expected, msg_data));
  BROKER_TEST(broker_set_insert(pq_a1_expected, msg_data));
  BROKER_TEST(broker_set_insert(pq_a2_expected, msg_data));
  msg_data = make_test_msg(node0, "", "pointless", topic_nobody, msg,
                           msgs_to_send + off++);
  msg_data = make_test_msg(node0, "0b", "node0 says: bye", topic_b, msg,
                           msgs_to_send + off++);
  BROKER_TEST(broker_set_insert(pq_b0_expected, msg_data));
  BROKER_TEST(broker_set_insert(pq_b1_expected, msg_data));
  msg_data = make_test_msg(node0, "0b", "node0 says: goodbye", topic_b, msg,
                           msgs_to_send + off++);
  BROKER_TEST(broker_set_insert(pq_b0_expected, msg_data));
  BROKER_TEST(broker_set_insert(pq_b1_expected, msg_data));

  msg_data = make_test_msg(node1, "1a", "node1 says: hi", topic_a, msg,
                           msgs_to_send + off++);
  BROKER_TEST(broker_set_insert(pq_a0_expected, msg_data));
  BROKER_TEST(broker_set_insert(pq_a1_expected, msg_data));
  msg_data = make_test_msg(node1, "1a", "node1 says: bye", topic_a, msg,
                           msgs_to_send + off++);
  BROKER_TEST(broker_set_insert(pq_a0_expected, msg_data));
  BROKER_TEST(broker_set_insert(pq_a1_expected, msg_data));
  msg_data = make_test_msg(node1, "1b", "node1 says: bbye", topic_b, msg,
                           msgs_to_send + off++);
  BROKER_TEST(broker_set_insert(pq_b0_expected, msg_data));
  BROKER_TEST(broker_set_insert(pq_b1_expected, msg_data));

  msg_data = make_test_msg(node2, "2a", "node2 says: hi", topic_a, msg,
                           msgs_to_send + off++);
  BROKER_TEST(broker_set_insert(pq_a0_expected, msg_data));
  BROKER_TEST(broker_set_insert(pq_a2_expected, msg_data));
  msg_data = make_test_msg(node2, "2a", "node2 says: bye", topic_a, msg,
                           msgs_to_send + off++);
  BROKER_TEST(broker_set_insert(pq_a0_expected, msg_data));
  BROKER_TEST(broker_set_insert(pq_a2_expected, msg_data));
  msg_data = make_test_msg(node2, "2b", "node2 says: bbye", topic_b, msg,
                           msgs_to_send + off++);
  BROKER_TEST(broker_set_insert(pq_b0_expected, msg_data));
  int i;

  for (i = 0; i < sizeof(msgs_to_send) / sizeof(test_message); ++i)
    BROKER_TEST(broker_endpoint_send(
      msgs_to_send[i].node, msgs_to_send[i].topic, msgs_to_send[i].msg));
