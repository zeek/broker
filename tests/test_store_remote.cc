...
dataset get_contents(const broker::store::frontend& store) {
  ...
  for (const auto& key : broker::store::keys(store)) {
    ...
    if (val)
      rval.insert(make_pair(key, move(*val)));
  }
...
void wait_for(const broker::store::frontend& f, broker::data k,
              bool exists = true) {
  while (broker::store::exists(f, k) != exists)
    usleep(1000);
}
...
int main(int argc, char** argv) {
  ...
  broker::table blue_pill{make_pair("1", "one"), make_pair("2", "two"),
                          make_pair(3, "three"),
                          make_pair("myrecord", myrecord)};
  dataset ds0;
  for (const auto& p : blue_pill)
    ds0.insert(p);
  ds0.insert(make_pair(blue_pill, "why?"));
  ds0.insert(make_pair("myrecord", myrecord));
  for (const auto& p : ds0)
    master.insert(p.first, p.second);
