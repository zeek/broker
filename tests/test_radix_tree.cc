...
static void test_prefix_of() {
  ...
  BROKER_TEST(check_match(t.prefix_of("one"), {make_pair("one", 1)}));
  BROKER_TEST(check_match(t.prefix_of("one-hundred"), {make_pair("one", 1)}));
  t["one-hundred"] = 100;
  BROKER_TEST(t.prefix_of("").empty());
  BROKER_TEST(t.prefix_of("nope").empty());
  BROKER_TEST(t.prefix_of("on").empty());
  ...
  BROKER_TEST(
    check_match(t.prefix_of("one-hundred"),
  ...
  BROKER_TEST(
    check_match(t.prefix_of("one"), {
                                      make_pair("", -1), make_pair("one", 1),
                                    }));
  BROKER_TEST(check_match(
    t.prefix_of("one-hundred"),
    {make_pair("", -1), make_pair("one", 1), make_pair("one-hundred", 100)}));
  BROKER_TEST(check_match(t.prefix_of("one-hundred-thousand"),
                          {make_pair("", -1), make_pair("one", 1),
                           make_pair("one-hundred", 100),
                           make_pair("one-hundred-thousand", 100000)}));
  BROKER_TEST(check_match(
    t.prefix_of("one-hundred-two"),
    {
      make_pair("", -1), make_pair("one", 1), make_pair("one-hundred", 100),
    }));
  BROKER_TEST(check_match(
    t.prefix_of("two-fifty-five"),
    {
      make_pair("", -1), make_pair("two", 2), make_pair("two-fifty", 250),
    }));
}
...
static void test_prefix_match() {
  ...
  BROKER_TEST(check_match(
    t.prefixed_by("api"),
    {
      make_pair("api", 6), make_pair("api.foe.fum", 3), make_pair("api.foo", 5),
  ...
  BROKER_TEST(
    check_match(t.prefixed_by("api.foo.bar"), {
  ...
  BROKER_TEST(check_match(
    t.prefix_of("api.foo.bar.baz"),
    {
      make_pair("api", 6), make_pair("api.foo", 5), make_pair("api.foo.bar", 1),
    }));
...
static void test_many_keys() {
  ...
  BROKER_TEST(check_match(
    matches, {
               make_pair("109", 109), make_pair("10", 10), make_pair("1", 1),
             }));

  matches = t.prefix_of("54321");
  BROKER_TEST(check_match(
    matches, {
               make_pair("543", 543), make_pair("54", 54), make_pair("5", 5),
             }));
...
ord$ air$ |    \  ary$ d$  |   \
  ...
  BROKER_TEST(
    check_match(matches, {make_pair("affair", 3), make_pair("afford", 1)}));
  matches = tree.prefixed_by("bi");
  BROKER_TEST(
    check_match(matches, {make_pair("binary", 5), make_pair("bind", 6)}));
  matches = tree.prefixed_by("a");
  BROKER_TEST(
    check_match(matches, {make_pair("apache", 0), make_pair("afford", 1),
  ...
  BROKER_TEST(
    check_match(matches, {make_pair("bind", 6), make_pair("binary", 5),
  ...
  BROKER_TEST(check_match(matches, {make_pair("bro", 42), make_pair("bros", 1),
                                    make_pair("brother", 2)}));
