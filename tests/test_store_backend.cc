...
int main(int argc, char** argv) {
  ...
  BROKER_TEST(db->push_left("ingredients", {"carrot", "potato"}, now()).stat
              == ok);
  BROKER_TEST(db->push_left("ingredients", {"cabbage", "beet"}, now()).stat
              == ok);
  BROKER_TEST(db->push_right("ingredients", {"beef", "pork"}, now()).stat
              == ok);
  BROKER_TEST(db->push_right("ingredients", {"chicken", "turkey"}, now()).stat
              == ok);
  ...
  BROKER_TEST(db->remove_from_set("cook names", "Katelyn Nacon", now()).stat
              == ok);
  ...
  BROKER_TEST(db->remove_from_set("cook names", "Layla Neal", now()).stat
              == ok);
  ...
  broker::set cook_names{
    "Ken DeLozier", "Tara Ochs",    "Justin Scott",
    "Morgan Burch", "Linda Miller", "Layla Neal",
  };
  ...
  broker::set more_cook_names{
    "Kayte Giralt", "Truman Orr", "Jayla James", "Cameron Markeles",
  };

  BROKER_TEST(db->insert("more cook names", more_cook_names));
  BROKER_TEST(db->add_to_set("more cook names", "Kayte Giralt", now()).stat
              == ok);
  BROKER_TEST(db->remove_from_set("more cook names", "Truman Orr", now()).stat
              == ok);
  more_cook_names.erase("Truman Orr");
  BROKER_TEST(db->add_to_set("more cook names", "Zack Shires", now()).stat
              == ok);
  more_cook_names.emplace("Zack Shires");
  BROKER_TEST(
    db->remove_from_set("more cook names", "Gwydion Lashlee-Walton", now()).stat
    == ok);
