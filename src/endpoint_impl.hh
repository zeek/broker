...
static void ocs_update(const caf::actor& q, peering::impl pi,
                       outgoing_connection_status::tag t,
                       std::string name = "") {
  peering p{std::unique_ptr<peering::impl>(new peering::impl(std::move(pi)))};
  caf::anon_send(q,
                 outgoing_connection_status{std::move(p), t, std::move(name)});
}
  ...
  endpoint_actor(caf::actor_config& cfg, const endpoint* ep,
    ...
    active = {
      ...
      [=](unsub_atom, const topic& t, const actor& p) {
        BROKER_DEBUG("endpoint." + name, "Peer '" + get_peer_name(p)
                                           + "' unsubscribed to '" + t + "'");
...
public:
  endpoint_proxy_actor(caf::actor_config& cfg, caf::actor local,
                       std::string endpoint_name, std::string addr,
                       uint16_t port, std::chrono::duration<double> retry_freq,
                       caf::actor ocs_queue)
    : caf::event_based_actor{cfg} {
    using namespace caf;
    using namespace std;
