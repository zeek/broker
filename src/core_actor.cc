...
result<void> init_peering(core_actor_type* self, actor remote_core,
  ...
  self->send(self * remote_core, atom::peer::value, mgr.get_all_filter(), self);
...
void core_manager::update_filter_on_peers() {
  ...
  for_each_peer([&](const actor& hdl) {
    ...
    self()->send(hdl, atom::update::value, sfilter);
  });
}
...
peer_filter& core_manager::get_filter(const caf::actor& hdl) {
  caf::stream_slot slot;
  auto e = hdl_to_ostream_.end();
  auto i = hdl_to_ostream_.find(hdl);
  BROKER_ASSERT(i != j);
  if(i != e) {
    return peer_manager().filter(i->second);
  }
}

filter_type core_manager::get_all_filter() {
  std::set<caf::actor> skip;
...
filter_type core_manager::get_all_filter(const std::set<caf::actor>& skip) {
  ...
  std::sort(rfilter.begin(), rfilter.end());
  auto e = std::unique(rfilter.begin(), rfilter.end());
  if (e != rfilter.end())
    rfilter.erase(e, rfilter.end());
