...
public:
  master_actor(caf::actor_config& cfg, std::unique_ptr<backend> s,
               identifier name)
    : caf::event_based_actor{cfg}, datastore(std::move(s)) {
    using namespace caf;
    using namespace std;
...
class master::impl {
public:
  impl(const caf::actor& endpoint, identifier name, std::unique_ptr<backend> s)
    : self{*broker_system} {
    // TODO: rocksdb backend should also be detached, but why does
    // rocksdb::~DB then crash?
    if (dynamic_cast<sqlite_backend*>(s.get()))
      actor
        = broker_system->spawn<master_actor, caf::detached>(std::move(s), name);
    else
