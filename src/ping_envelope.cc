...
envelope_ptr ping_envelope::with(endpoint_id new_sender, endpoint_id new_receiver) {
  using decorator_t = envelope::decorator<ping_envelope>;
...
ping_envelope_ptr ping_envelope::make(const endpoint_id& sender,
                                      ...
                                      const std::byte* payload,
                                      size_t payload_size) {
  return make_intrusive<default_ping_envelope>(sender, receiver, payload,
                                               payload_size);
}
...
expected<envelope_ptr> ping_envelope::deserialize(
  ...
  using impl_t = envelope::deserialized<ping_envelope>;
