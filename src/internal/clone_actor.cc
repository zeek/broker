...
void clone_state::operator()(set_command& x) {
  ...
  publisher_id publisher{facade(master.node()), master.id()};
  ...
  return {
    ...
    [=](atom::get, atom::keys) -> caf::result<data> {
      if ( self->state.is_stale )
        return {caf::make_error(ec::stale_data)};
    ...
    [=](atom::get, atom::keys, request_id id) {
      if ( self->state.is_stale )
        return caf::make_message(caf::make_error(ec::stale_data), id);
    ...
    [=](atom::get, const data& key) -> caf::result<data> {
      ...
      return to_caf_res(std::move(result));
    },
    [=](atom::get, const data& key, const data& aspect) -> caf::result<data> {
      ...
      auto i = self->state.store.find(key);
      if (i != self->state.store.end())
        result = visit(detail::retriever{aspect}, i->second);
    ...
    [=](atom::get, const data& key, request_id id) {
      ...
      if (i != self->state.store.end()) {
        ...
        result = caf::make_message(caf::make_error(ec::no_such_key), id);
    ...
    [=](atom::get, const data& key, const data& aspect, request_id id) {
      ...
      auto i = self->state.store.find(key);
      if (i != self->state.store.end()) {
        auto x = visit(detail::retriever{aspect}, i->second);
        BROKER_INFO("GET" << key << aspect << "with id" << id << "->" << x);
        if (x)
          result = caf::make_message(*x, id);
        else
          result = caf::make_message(native(x.error()), id);
      } else {
        result = caf::make_message(caf::make_error(ec::no_such_key), id);
    ...
    [=](caf::stream<command_message> in) {
      attach_stream_sink(
        ...
        [=](caf::unit_t&, command_message y) {
          ...
          if (holds_alternative<snapshot_sync_command>(cmd)) {
