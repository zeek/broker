...
#include "broker/internal/central_dispatcher.hh"
...
template <class T>
[[nodiscard]] constexpr bool is_eligible(detail::item_scope x) noexcept {
...
public:
  ...
  bool enqueue(detail::item_scope scope, caf::span<const node_message> messages,
    ...
    if (is_eligible<T>(scope)) {
      detail::prefix_matcher matches_filter;
      auto old_size = cache_.size();
      for (const auto& msg : messages) {
        if (is_eligible<T>(msg) && matches_filter(filter_, msg)) {
          if constexpr (std::is_same<T, data_message>::value) {
            cache_.emplace_back(get<data_message>(msg.content));
          } else if constexpr (std::is_same<T, command_message>::value) {
            cache_.emplace_back(get<command_message>(msg.content));
          } else {
...
public:
  ...
  bool enqueue(const unipath_manager* source, detail::item_scope scope,
...
public:
  ...
  bool enqueue(const unipath_manager*, detail::item_scope,
...
public:
  ...
  explicit unipath_manager_in(central_dispatcher* dispatcher,
    ...
    if (!std::is_same<T, node_message>::value
        || get_or(cfg, "broker.forward", true)) {
      ttl_ = get_or(cfg, "broker.ttl", defaults::ttl);
    } else {
  ...
  void handle_batch(std::vector<node_message>& xs) {
    ...
    if (auto added = pending_.size() - old_size; added > 0) {
      ...
      super::dispatcher_->enqueue(this, detail::item_scope::global, ys);
    }
  }
  ...
  void handle_batch(std::vector<MessageType>& xs) {
    ...
    if (auto added = pending_.size() - old_size; added > 0) {
      auto ys = caf::make_span(std::addressof(pending_[old_size]), added);
      super::dispatcher_->enqueue(this, detail::item_scope::remote, ys);
    }
  }
  ...
  int32_t acquire_credit(caf::inbound_path* in, int32_t desired) override {
    ...
    auto is_shipped = [](const node_message& msg) {
      if (is_data_message(msg)) {
        return get<data_message>(msg.content).unique();
      } else {
        return get<command_message>(msg.content).unique();
      }
    };
