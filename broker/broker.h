...
/// An opaque data value, which may be any of the types in 
/// @see broker_data_type.
struct broker_data;
...
/// the current ordering of the table's contents), but the associated value
/// may.
typedef struct {
...
/// An entry in a table which may only be used to inspect the key and
/// associated value, but not modify them.
typedef struct {
  const broker_data* key;
...
/// @param true_or_false pass in zero to set *b* to `false`, or not-zero to set
/// it to `true`.
...
/// @return new address data whose value is copied from the argument.  The
...
/// @return new subnet data whose value is copied from the argument.  The
/// caller must manage the memory of the returned object.
/// Or returns null if sufficient memory could not be allocated.
...
/// to the current number of elements in the vector.  The vector is
/// automatically resized to fit the new element and other contents are shifted
/// right. 
/// @return not-zero if the call succeeds, or zero if sufficient memory could
/// not be allocated.
...
/// endpoints.
const broker_peering* broker_outgoing_connection_status_peering(
  const broker_outgoing_connection_status*);
...
const char* broker_outgoing_connection_status_peer_name(
  const broker_outgoing_connection_status*);
...
const char* broker_incoming_connection_status_peer_name(
  const broker_incoming_connection_status*);
...
/// @see broker::endpoint::peer(std::string, uint16_t,
...
/// @return not-zero on success, else zero if failed to allocate memory for
/// copying the topic string.
int broker_endpoint_unadvertise(broker_endpoint* e, const broker_string* topic);
...
broker_store_result_tag broker_store_result_which(const broker_store_result* r);
...
broker_store_query* broker_store_response_get_query(broker_store_response* r);
...
broker_store_result* broker_store_response_get_result(broker_store_response* r);
...
/// @param e the endpoint to attach the master store to.
/// @param name a unique name to give the master data store.
/// @param b the backend to use.  On a successful call, it assumes ownership of
/// the backend.  The caller may call "delete" on it whenever, but that is the
/// only valid operation that they may perform.
/// @return a new store object for the caller to manage.
/// Or null if sufficient memory could not be allocated.
broker_store_frontend*
