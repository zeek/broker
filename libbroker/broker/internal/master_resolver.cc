...
caf::behavior master_resolver(master_resolver_actor* self) {
  self->set_error_handler([=](caf::error&) {
    if (--self->state.remaining_responses == 0) {
      log::store::error("resolver-failed", "resolver failed to find a master");
  ...
  return {
    [=](const std::vector<caf::actor>& peers, const std::string& name,
        caf::actor& who_asked) {
      log::store::debug("resolver-start", "resolver starts looking for: {}",
                        name);
      for (auto& peer : peers)
    ...
    [=](caf::actor& master) {
      log::store::debug("resolver-success", "resolver found master: {}",
                        master);
