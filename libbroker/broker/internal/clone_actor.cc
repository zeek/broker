...
event::component_type clone_state::component() const noexcept {
  return event::component_type::clone_store;
}
...
clone_state::clone_state(caf::event_based_actor* ptr,
  ...
  log::clone_store::info("attached", "attached clone {} to {}", id, store_name);
}
...
void clone_state::dispatch(const command_message& msg) {
  log::clone_store::debug("dispatch", "received command {}", msg);
  ...
  if (input.initialized() && cmd.sender != input.producer()) {
    log::clone_store::warning("unknown-sender",
  ...
  auto is_receiver = [this, &cmd] {
    if (cmd.receiver == id) {
      ...
      if (cmd.receiver) {
        log::clone_store::debug("discard-wrong-receiver",
                                "clone received message for {}", cmd.receiver);
      } else {
        log::clone_store::debug("discard-broadcast",
  ...
  switch (tag) {
    ...
    case command_tag::producer_control: {
      ...
      switch (type) {
        case internal_command::type::ack_clone_command: {
          if (master_id) {
            if (cmd.sender == master_id) {
              log::clone_store::debug("repeated-ack-clone",
                                      "drop repeated ack_clone from {}",
                                      master_id);
            } else {
              log::clone_store::error(
          ...
          if (input.handle_handshake(cmd.sender, inner.offset,
            ...
            log::clone_store::debug("received-ack-clone",
        ...
        default: {
          log::clone_store::error(
            "unexpected-producer-control",
            "received unexpected producer control message: {}", cmd);
        }
      }
      break;
    }
    default: {
      ...
      if (!is_receiver()) {
        ...
        log::clone_store::debug(
          "consumer-control-unknown-channel",
      ...
      switch (type) {
          ...
          if (auto i = output_opt->find_path(entity_id::nil());
            ...
            if (master_id) {
              if (master_id == cmd.sender) {
                i->hdl = cmd.sender;
                log::clone_store::debug("received-write-ack",
              ...
              log::clone_store::debug("received-write-ack-and-set-master",
        ...
        default: {
          log::clone_store::error("unexpected-consumer-control",
                                  "received bogus consumer control message: {}",
                                  cmd);
        }
      }
    }
...
void clone_state::consume(put_command& x) {
  log::clone_store::debug("put-command",
...
void clone_state::consume(put_unique_result_command& cmd) {
  log::clone_store::debug("put-unique-result-command", "clone received: {}",
                          cmd);
...
void clone_state::consume(erase_command& x) {
  log::clone_store::debug("erase-command",
                          "clone received erase command for key {}", x.key);
  if (store.erase(x.key) != 0)
    emit_erase_event(x.key, x.publisher);
}

void clone_state::consume(expire_command& x) {
  log::clone_store::debug("expire-command",
...
void clone_state::consume(clear_command& x) {
  log::clone_store::info("clear-command", "clone received clear command");
  for (auto& kvp : store)
...
error clone_state::consume_nil(consumer_type* src) {
  log::clone_store::error("out-of-sync",
...
void clone_state::close(consumer_type* src, const error& reason) {
  log::clone_store::info("close", "clone is closing: {}", reason);
...
void clone_state::send(producer_type* ptr, const entity_id& dst,
  ...
  log::clone_store::debug("send-event",
                          "send event with seq {} and type {} to {}",
...
void clone_state::send(producer_type* ptr, const entity_id&,
  ...
  log::clone_store::debug("send-handshake",
                          "send attach_writer_command with offset {}",
                          what.offset);
  auto msg = make_command_message(
    master_topic,
...
void clone_state::send(producer_type* ptr, const entity_id&,
  ...
  log::clone_store::debug("send-retransmit-failed",
...
void clone_state::broadcast(producer_type* ptr, channel_type::heartbeat what) {
  ...
  for (auto& path : ptr->paths()) {
    if (path.acked == 0) {
      log::clone_store::debug("re-send-handshake", "re-send handshake to {}",
                              path.hdl);
      send(ptr, path.hdl,
           channel_type::handshake{path.offset, ptr->heartbeat_interval()});
    }
  }
  log::clone_store::debug("send-keepalive", "send keepalive to master {}",
                          master_id);
  auto msg = make_command_message(
    master_topic,
...
void clone_state::broadcast(producer_type* ptr,
  ...
  log::clone_store::debug("broadcast-event",
...
void clone_state::drop(producer_type*, const entity_id&, ec reason) {
  log::clone_store::debug("drop", "drop producer with reason {}", reason);
  // TODO: see comment in close()
}

void clone_state::handshake_completed(producer_type*, const entity_id&) {
  log::clone_store::debug("handshake-completed",
                          "completed producer handshake for store {}",
                          store_name);
}
...
void clone_state::set_store(std::unordered_map<data, data> x) {
  log::clone_store::debug("set-store", "set store values with {} entries",
                          x.size());
...
void clone_state::start_output() {
  if (output_opt) {
    log::clone_store::warning("repeat-start-output",
                              "repeated calls to clone_state::start_output");
    return;
  }
  BROKER_ASSERT(master_id);
  log::clone_store::debug("add-output-channel",
                          "clone {} adds an output channel", id);
...
void clone_state::send_to_master(internal_command_variant&& content) {
  if (output_opt) {
    BROKER_ASSERT(master_id);
    log::clone_store::debug("send-to-master",
                            "send command of type {} to master",
                            content.index());
    auto& out = *output_opt;
    auto msg = make_command_message(
      master_topic,
    ...
    log::clone_store::debug("buffer-to-master",
                            "buffer command of type {} for master",
                            content.index());
  ...
  return super::make_behavior(
    ...
    [=](atom::local, internal_command_variant& content) {
      if (auto inner = get_if<put_unique_command>(&content)) {
        if (inner->who) {
          log::clone_store::debug("received-put-unique",
    ...
    [=](atom::tick) {
      ...
      if (sync_timeout) {
        if (has_master()) {
          ...
          log::clone_store::error("no-master", "unable to find a master for",
                                  store_name);
